...
site <- dplyr::select(hor, id_prof, x, y, date) %>% unique()
hor <- dplyr::select(hor, id_prof, id_hor, top:cec)
...
## 3.1 - select columns --------------------------------------------------------
hor <- dplyr::select(hor, ProfID, HorID, top, bottom, ph=ph_h2o, k, soc, clay, bd, cec)
...
aqp::metadata(clean_prof)$removed.profiles
...
dat <- dplyr::select(dat, ProfID, HorID, x, y, date, top, bottom, ph:cec )
...
names(dat)
dat <- dplyr::select(dat, ProfID, HorID, x, y, top, bottom, ph, k, soc, clay, bd, cec)
