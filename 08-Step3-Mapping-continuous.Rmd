...
## 1.1 - Load covariates ----------------------------------------
covs <- rast("01-Data/covs/Covariates.tif") # match case of the 
#file name
ncovs <- names(covs)
...
```{r, eval = F}
# Reproject point coordinates to match coordinate system 
# of covariates
...
```{r, eval = F}
  ...
  fs_bor <-
  Boruta(y = d[,soilatt], x = d[-1], maxRuns = 100, doTrace = 1)
  ...
  (fs_vars <- 
     getSelectedAttributes(fs_bor, withTentative = TRUE))
...
```{r borutaplot, echo = FALSE, fig.cap = "Covariates selecction for Total Nitrogen using Boruta algorithm.", out.width='100%'}
...
```{r cv, echo = FALSE, fig.cap = "Schematic representation of the repeated cross-validation process.", out.width='100%'}
...
model_rn <-
readRDS(
'Digital-Soil-Mapping/02-Outputs/models/ranger_model_soc_0_30.rds'
  )
...
```{r, eval = T, message = F}
  ...
  # ggsave(g1, filename = 
#paste0("02-Outputs/residuals_",soilatt,".png"), scale = 1,
...
kable(booktabs = T, dt, col.names = gsub("[.]", " ", 
            names(dt)), caption = 'Accuracy statistics.') %>%
...
# tile <-
#makeTiles(r, t,
#overwrite=TRUE,filename="02-Outputs/tiles/tiles.tif")
...
```{r, eval = F}
  ...
  # loop to predict soilatt on each tile
    ...
    # create a function to extract the predited values from 
    ...
    terra::interpolate(covst, 
                       ...
                       what=sd,
                       filename = 
                        paste0("02-Outputs/tiles/soilatt_tiles/",
...
```{r, eval = F}
  ...
  plot(c(pred_mean, pred_sd), main = paste(c("mean","sd"), 
                                           soilatt), 
...
pred_mean <- 
rast(
  "Digital-Soil-Mapping/02-Outputs/noMaskedMaps/mean_n_0_30.tif")
pred_sd <- rast(
  "Digital-Soil-Mapping/02-Outputs/noMaskedMaps/sd_n_0_30.tif")
plot(c(pred_mean, pred_sd), main = paste(c("mean","sd"),
                                         soilatt), 
       col = hcl.colors(100, "Viridis"))
```
...
```{r, eval = F}
  ...
  # plot(pred_sd)
  plot(pred_sd/pred_mean*100, main = 
         paste("Coeficient of variation", soilatt), 
       col = hcl.colors(100, "Viridis"))
