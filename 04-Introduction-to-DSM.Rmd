...
A broad range of modelling approaches coexist in order to establish quantitative relationships between environmental covariates and the target soil properties to be mapped. Traditionally, multiple linear regression models can be used to quantify the relationships which continues to be the most applied mapping method to map for instance soil organic carbon [@lamichhane2019]. Regression Kriging methods use linear regressions and in addition to that interpolates the residuals of the regression for spatial autocorrelation [@yigini2018]. 
...
knitr::include_graphics("images/workflow lat-long data.png")
```
