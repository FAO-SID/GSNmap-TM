...
Assessing the uncertainty related to a prediction means to compare the difference between observed and predicted values. Hence, it is necessary to extracted observed and predicted values and store them in two separate R objects. Next, both values are combined to a dataframe.
...
This dataframe is used to plot observed values on the x-axis and predicted values on the y-axis as a scatterplot.
...
Additionally, it is necessary to calculate standard metrics of error estimation. The function eval() below returns values for the mean error (ME), root mean-squared error (RMSE), mean absolute error (MAE), the squared pearson correlation coefficient, the concordance correlation coefficient, scale shift and location shift relative to scale.
